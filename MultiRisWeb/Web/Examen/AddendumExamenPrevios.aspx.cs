...
{
    public partial class AddendumExamenPrevios : System.Web.UI.Page
    {
        ...
        public static string ObtenerInformesPrevios(string id_paciente, int id_institucion, string codexamen)
        {
            ...
            if (byId.id_institucion > 0)
            {
                ...
                if (byId.id_tipo_conexion == 1)
                    dataTable = AddendumExamenPrevios.normalizaDataTableEstudiosRelacionados(new ConsumirApi().ApiObtenerDataTable(methodAndInstitucion.url, methodAndInstitucion.metodo, AddendumExamenPrevios.jsonEstudiosRelacionados(id_paciente), id_institucion), id_institucion, id_paciente, codexamen);
                else if (byId.id_tipo_conexion == 2)
                {
                    ConsumirWS consumirWs = new ConsumirWS();
                    dataTable = AddendumExamenPrevios.normalizaDataTableEstudiosRelacionados(ConsumirWS.GetEstudiosRelacionados(id_paciente, id_institucion), id_institucion, id_paciente, codexamen);
                }
            }
