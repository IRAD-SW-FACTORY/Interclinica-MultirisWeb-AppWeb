                ...
                foreach (VisorDomain visorDomain in (IEnumerable<VisorDomain>)allForState)
                {
                    ...
                    if (flag && institucionAndVisor.id_institucion_visor > 0)
                    {
                        if (visorDomain.id_visor == 2)
                        {
                            ...
                            str = str + "&nbsp;<a href='" + institucionAndVisor.url + codexamen + "&add=true" + "' target='_blank' onclick='LogMeddream(" + institucionCodExamen.id_ris_examen + ");' title='Ver con " + visorDomain.nombre + "' ><img class='imgvisor' src='../icon/" + visorDomain.icono + "' ></a>";
                        }
                ...
                foreach (VisorDomain visorDomain in (IEnumerable<VisorDomain>)allForState)
                {
                    ...
                    if (flag && institucionAndVisor.id_institucion_visor > 0)
                    {
                        if (visorDomain.id_visor == 2)
                        {
                            ...
                            str = str + "&nbsp;<a href='" + institucionAndVisor.url + codexamen + "&add=true" + "' target='_blank' title='Ver con " + visorDomain.nombre + "' ><img class='imgvisor' src='../icon/" + visorDomain.icono + "' ></a>";
                        }
                        else if (visorDomain.id_visor != 6)
          ...
          bool estudio_actual)
        {
            ...
            foreach (VisorDomain visorDomain in (IEnumerable<VisorDomain>)allForState)
            {
                ...
                if (byIdInstitucion.id_meddreams > 0 && byIdInstitucion.id_estado == 1 && visorDomain.id_visor == 2)
                {
                    ...
                    str = institucionAndVisor.url+ cod_examen_actual + "&add=true";
                }
            }
            return str;
        }
